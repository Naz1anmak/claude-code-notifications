...
# Claude Code CLI — десктоп-уведомления

Скилл-визард, который настраивает нативные OS-уведомления для [Claude Code **CLI**](https://docs.claude.com/en/docs/claude-code) на **macOS**, **Linux** и **Windows**: всплывашка, когда Claude закончил ход, когда инструменту нужно подтверждение, или когда ждёт ответа на интерактивный вопрос.

> У десктоп-приложения Claude Code свои встроенные уведомления. Этот скилл — про **терминальный CLI**, который сам по себе ничего системе не показывает.
...
По умолчанию CLI молчит — если ты переключился в браузер пока он работает, ты упустишь момент когда он закончил или попросил подтверждение на команду. Этот скилл расставляет:
